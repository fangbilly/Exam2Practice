...
class Box(object):
    ...
    def double(self):
        ...
        #    ** TWO **   LINES OF CODE.
        ################################################################
        additional_str = self.contents
        return self.append_string(additional_str)
    ...
    def shrink(self, new_volume):
        ...
        new_self = ''
        clipped_self = ''
        new_self_size = min(len(self.contents), new_volume)
        for k in range(new_self_size):
            new_self += self.contents[k]
        for k in range(new_self_size, len(self.contents)):
            clipped_self += self.contents[k]
        self.contents = new_self
